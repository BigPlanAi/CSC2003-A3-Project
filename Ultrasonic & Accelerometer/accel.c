...
float getAngle()
{
    ...
    float accAngleZ = (atan(-1 * accX / sqrt(pow(accY, 2) + pow(accZ, 2))) * 180 / M_PI);
...
// Unsure what happens if only one wheel goes over a hump -
...
float calculateHumpHeight()
{
    ...
    hump_horizontal_dist = (Ultrasonic_getDistanceFromFrontSensor() - hump_horizontal_dist) * 0.5f;

    height = hump_horizontal_dist * tan(getAngle() * M_PI / 180);
