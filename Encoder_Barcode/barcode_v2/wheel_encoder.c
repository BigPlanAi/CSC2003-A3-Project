...
//        wheel_Left_Time_Counter += TICKPERIOD;
//    }
//    if (wheel_Left_Time_Counter > PAUSE_PERIOD){  // After 0.5s reset velocity
//        wheel_Left_Velocity= 0.0;
//        wheel_Left_Time_Counter = 0;
//    }
//    if (wheel_Right_Time_Counter > PAUSE_PERIOD){ // After 0.5s reset velocity
//        wheel_Right_Velocity=  0.0;
//        wheel_Right_Time_Counter = 0;
//
//    }
...
/*
 * wheel_Encoder_Left_IRQ()  Calculates the wheel velocity of the left wheel
 *
 * - Applies exponential moving average on wheel velocity to smoothen the reading
 * - Called in Interrupt handler of Left Wheel encoder of pin port
 * - Wheel velocity = (distance of 1 notch)  / (time to complete one notch)
 * - Clears time counter everybs successful call
 */
//void wheel_Encoder_Left_IRQ(void){
//    static uint16_t calculate_velocity = 0;
//    uint32_t status = GPIO_getEnabledInterruptStatus(WHEEL_ENCODER_LEFT_PORT);
//    // Check pin
//    if(status & WHEEL_ENCODER_LEFT_PIN)
//    {
...
//        leftWheelCount++;
//        if ((wheel_Left_Time_Counter   < 1) || (calculate_velocity == 0)){
//
//            wheel_Left_Time_Start=  Timer_A_getCounterValue(TIMER_A1_BASE);
//            wheel_Left_Time_Counter =  1;
...
//        else{
//
//            uint32_t time_taken = wheel_Left_Time_Counter + Timer_A_getCounterValue(TIMER_A1_BASE) - wheel_Left_Time_Start ;
//            if (wheel_Left_Velocity <  0.1){
//
//                wheel_Left_Velocity = WHEEL_CIRC_PER_NOTCH  /   (time_taken * TIME_PER_CYCLE) ;
//            }
//            else{
//                wheel_Left_Velocity =( WHEEL_CIRC_PER_NOTCH  /   (time_taken * TIME_PER_CYCLE) * MOVING_AVG_DELTA ) +  wheel_Left_Velocity * (1 - MOVING_AVG_DELTA);
//
//            }
...
/*
 * wheel_Encoder_Right_IRQ()  Calculates the wheel velocity of the right wheel when called
 *
 * - Applies exponential moving average on wheel velocity to smoothen the reading
 * - Called in Interrupt handler of Right Wheel encoder of pin port
 * - Wheel velocity = (distance of 1 notch)  / (time to complete one notch)
 * - Clears time counter at the end of function
 */
//void wheel_Encoder_Right_IRQ(void){
//    static uint16_t calculate_velocity = 0;
//
...
//    {
//        rightWheelCount++;
//        uint32_t time_taken =  wheel_Right_Time_Counter + Timer_A_getCounterValue(TIMER_A1_BASE) - wheel_Right_Time_Start ;
//        //P3->IES ^=  WHEEL_ENCODER_RIGHT_PIN;
////        if (startRightWheel == '1'){
////            rightWheelCount++;
////        }
//
//        if ( (wheel_Right_Time_Counter <1)   || (calculate_velocity == 0)){
//            wheel_Right_Time_Start = Timer_A_getCounterValue(TIMER_A1_BASE);
//            wheel_Right_Time_Counter =  1;
//            calculate_velocity= 1;
//            //printf("\n1");
...
//        else{
//            uint32_t time_taken =  wheel_Right_Time_Counter + Timer_A_getCounterValue(TIMER_A1_BASE) - wheel_Right_Time_Start ;
//            if (wheel_Right_Velocity <  0.1){
//               // printf("\n2 : ");
//                wheel_Right_Velocity = WHEEL_CIRC_PER_NOTCH  / (time_taken * TIME_PER_CYCLE);
//            }
//            else{
//               // printf("\n3: ");
//                wheel_Right_Velocity =  ((WHEEL_CIRC_PER_NOTCH  / (time_taken * TIME_PER_CYCLE) ) - wheel_Right_Velocity) *  MOVING_AVG_DELTA  + wheel_Right_Velocity;
//                //wheel_Right_Velocity =  (WHEEL_CIRC_PER_NOTCH  / (time_taken * TIME_PER_CYCLE) * MOVING_AVG_DELTA ) +  wheel_Right_Velocity * (1 -MOVING_AVG_DELTA);
//            }
//            //printf("%0.2f",wheel_Right_Velocity );
//            calculate_velocity= 0;
//
//        }
//        GPIO_clearInterruptFlag(WHEEL_ENCODER_RIGHT_PORT, status);
